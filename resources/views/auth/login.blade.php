...
<div class="flex flex-row">
    ...
    <div class="pl-4">
        ...
        <h2 class="text-rose-800 font-custom2 text-xl">Your daily dose of paw-sitivity !</h2>
    </div>
</div>
    ...
    <form method="POST" action="{{ route('login') }}">
        ...
        <div class="block mt-4">
            <label for="remember_me" class="inline-flex items-center">
                ...
                <span class="ml-2 text-sm text-pink-950">{{ __('Remember me') }}</span>
            </label>
        </div>

        <div class="flex items-center justify-end mt-4">
            @if (Route::has('password.request'))
                <a class="underline text-sm text-pink-950 hover:text-gray-900 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-pink-950" href="{{ route('password.request') }}">
                    {{ __('Forgot your password?') }}
                </a>
            @endif

            <x-primary-button class="ml-3 bg-red-500 text-yellow-50 ">
                {{ __('Log in') }}
            </x-primary-button>
        </div>
    </form>
